 ...
 * parse to JSON
 * *************/
const list_to_array = xs => is_null(xs)
    ? []
...
// and wrap in a block
const parse_to_json = program_text => ({ tag: "blk",
...
const is_closure = x => x !== null &&
    typeof x === "object" &&
    x.tag === 'closure';
const is_builtin = x => x !== null &&
    typeof x === "object" &&
    x.tag == 'builtin';
// catching closure and builtins to get short displays
const value_to_string = x => is_closure(x)
    ? '<closure>'
    : is_builtin(x)
        ? '<builtin: ' + x.sym + '>'
        : stringify(x);
...
const microcode = {
    ...
    app: cmd => push(A, { tag: 'app_i', arity: cmd.args.length }, ...cmd.args, // already in reverse order, see ast_to_json
    cmd.fun),
    ...
    arr_len: cmd => push(A, { tag: 'arr_len_i' }, cmd.expr),
    arr_lit: cmd => push(S, cmd.elems),
    //
    // statements
    //
    ...
    let: cmd => push(A, { tag: 'lit', val: undefined }, { tag: 'pop_i' }, { tag: 'assmt', sym: cmd.sym, expr: cmd.expr }),
    const: cmd => push(A, { tag: "lit", val: undefined }, { tag: 'pop_i' }, { tag: 'assmt', sym: cmd.sym, expr: cmd.expr }),
    ret: cmd => push(A, { tag: 'reset_i' }, cmd.expr),
    fun: cmd => push(A, { tag: 'const',
        sym: cmd.sym,
        expr: { tag: 'lam', prms: cmd.prms, body: cmd.body } }),
    while: cmd => push(A, { tag: 'lit', val: undefined }, { tag: 'while_i', pred: cmd.pred, body: cmd.body }, cmd.pred),
    //
    // instructions
    //
    reset_i: cmd => A.pop().tag === 'mark_i' // mark found?  
        ? null // stop loop
        : push(A, cmd),
    ...
    while_i: cmd => S.pop()
        ...
        cmd.pred, { tag: 'pop_i' }, // pop body value
        cmd.body)
        : null,
    env_i: cmd => E = cmd.env,
    arr_acc_i: cmd => {
...
const all_except_last = xs => is_null(tail(xs))
    ? null
    : pair(head(xs), all_except_last(tail(xs)));
const command_to_string = cmd => (cmd.tag === 'env_i')
    ? '{ tag: "env_i", env: ...}'
    : JSON.stringify(cmd);
const debug = (cmd) => {
...
//
//# sourceMappingURL=original_eval.js.map