 ...
 */
exports.getImportedLocalModulePaths = (program, currentFilePath) => {
    ...
    importDeclarations.forEach((importDeclaration) => {
        ...
        if (!removeNonSourceModuleImports_1.isSourceModule(modulePath)) {
...
const parseProgramsAndConstructImportGraph = (files, entrypointFilePath, context) => {
    ...
    const parseFile = (currentFilePath) => {
        ...
        const parserOptions = shouldAddSourceFileToAST
            ...
            : {};
        const program = parser_1.parse(code, context, parserOptions);
        if (program === null) {
            return;
        }
        programs[currentFilePath] = program;
        const importedLocalModulePaths = exports.getImportedLocalModulePaths(program, currentFilePath);
...
const preprocessFileImports = (files, entrypointFilePath, context) => {
    ...
    const entrypointProgramInvokedFunctionResultVariableNameToImportSpecifiersMap = transformProgramToFunctionDeclaration_1.getInvokedFunctionResultVariableNameToImportSpecifiersMap(entrypointProgramModuleDeclarations, entrypointDirPath);
    const entrypointProgramAccessImportStatements = transformProgramToFunctionDeclaration_1.createAccessImportStatements(entrypointProgramInvokedFunctionResultVariableNameToImportSpecifiersMap);
    ...
    for (const [filePath, program] of Object.entries(programs)) {
        ...
        const functionDeclaration = transformProgramToFunctionDeclaration_1.transformProgramToFunctionDeclaration(program, filePath);
    ...
    topologicalOrderResult.topologicalOrder.forEach((filePath) => {
        ...
        const functionName = filePaths_1.transformFilePathToValidFunctionName(filePath);
        const invokedFunctionResultVariableName = filePaths_1.transformFunctionNameToInvokedFunctionResultVariableName(functionName);
        ...
        const invokedFunctionResultVariableDeclaration = contextSpecificConstructors_1.createInvokedFunctionResultVariableDeclaration(functionName, invokedFunctionResultVariableName, functionParams);
    ...
    removeExports_1.removeExports(preprocessedProgram);
    ...
    removeNonSourceModuleImports_1.removeNonSourceModuleImports(preprocessedProgram);
    ...
    hoistAndMergeImports_1.hoistAndMergeImports(preprocessedProgram);
