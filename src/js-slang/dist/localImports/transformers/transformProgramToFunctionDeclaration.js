...
exports.getInvokedFunctionResultVariableNameToImportSpecifiersMap = (nodes, currentDirPath) => {
    ...
    nodes.forEach((node) => {
        ...
        if (removeNonSourceModuleImports_1.isSourceModule(importSource)) {
            return;
        }
        ...
        const importFunctionName = filePaths_1.transformFilePathToValidFunctionName(importFilePath);
        ...
        const invokedFunctionResultVariableName = filePaths_1.transformFunctionNameToInvokedFunctionResultVariableName(importFunctionName);
...
const getDefaultExportExpression = (nodes, exportedNameToIdentifierMap) => {
    ...
    nodes.forEach((node) => {
        ...
        if (typeGuards_1.isDeclaration(node.declaration)) {
            const identifier = getIdentifier(node.declaration);
            if (identifier === null) {
                return;
            }
...
exports.createAccessImportStatements = (invokedFunctionResultVariableNameToImportSpecifiersMap) => {
        ...
        importSpecifiers.forEach((importSpecifier) => {
            ...
            switch (importSpecifier.type) {
                case 'ImportSpecifier':
                    importDeclaration = contextSpecificConstructors_1.createImportedNameDeclaration(invokedFunctionResultVariableName, importSpecifier.local, importSpecifier.imported.name);
                    break;
                case 'ImportDefaultSpecifier':
                    importDeclaration = contextSpecificConstructors_1.createImportedNameDeclaration(invokedFunctionResultVariableName, importSpecifier.local, localImport_prelude_1.defaultExportLookupName);
                    break;
...
const createReturnListArguments = (exportedNameToIdentifierMap) => {
    return Object.entries(exportedNameToIdentifierMap).map(([exportedName, identifier]) => {
        const head = baseConstructors_1.createLiteral(exportedName);
        const tail = identifier;
        return contextSpecificConstructors_1.createPairCallExpression(head, tail);
    });
};
const removeDirectives = (nodes) => {
    return nodes.filter((node) => !typeGuards_1.isDirective(node));
};
const removeModuleDeclarations = (nodes) => {
    const statements = [];
    nodes.forEach((node) => {
        if (typeGuards_1.isStatement(node)) {
 ...
 */
exports.transformProgramToFunctionDeclaration = (program, currentFilePath) => {
    ...
    const invokedFunctionResultVariableNameToImportSpecifiersMap = exports.getInvokedFunctionResultVariableNameToImportSpecifiersMap(moduleDeclarations, currentDirPath);
    const accessImportStatements = exports.createAccessImportStatements(invokedFunctionResultVariableNameToImportSpecifiersMap);
    ...
    const defaultExport = defaultExportExpression !== null && defaultExportExpression !== void 0 ? defaultExportExpression : baseConstructors_1.createLiteral(null);
    const namedExports = contextSpecificConstructors_1.createListCallExpression(createReturnListArguments(exportedNameToIdentifierMap));
    const returnStatement = baseConstructors_1.createReturnStatement(contextSpecificConstructors_1.createPairCallExpression(defaultExport, namedExports));
    ...
    const functionName = filePaths_1.transformFilePathToValidFunctionName(currentFilePath);
    ...
    return baseConstructors_1.createFunctionDeclaration(functionName, functionParams, functionBody);
};
