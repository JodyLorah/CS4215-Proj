...
let context = context_1.mockContext(types_1.Chapter.SOURCE_3, types_1.Variant.TYPED);
beforeEach(() => {
    context = context_1.mockContext(types_1.Chapter.SOURCE_3, types_1.Variant.TYPED);
});
describe('array type', () => {
    ...
    `;
        ...
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 4: Type '(number | string)[]' is not assignable to type 'boolean[]'."`);
    });
...
describe('variable assignment', () => {
    ...
    `;
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 2: Cannot assign to 'x' as it is a constant."`);
    });
});
...
describe('stream type', () => {
    ...
    it('type alias with the same name cannot be declared', () => {
        ...
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 1: Type alias 'Stream' has already been declared."`);
    });
});
