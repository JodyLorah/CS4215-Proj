...
describe('parse tree types', () => {
    it('prelude has no errors', () => {
        parser_1.parse('const x = 1;', context);
...
describe('parse', () => {
    ...
    `;
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 2: Type 'number' is not assignable to type 'string'."`);
    });
    it('returns Program | Statement', () => {
        const code = "const x: number = parse('1;');";
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 1: Type 'Program | Statement' is not assignable to type 'number'."`);
    });
});
