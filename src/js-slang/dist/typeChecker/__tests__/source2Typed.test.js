...
let context = context_1.mockContext(types_1.Chapter.SOURCE_2, types_1.Variant.TYPED);
beforeEach(() => {
    context = context_1.mockContext(types_1.Chapter.SOURCE_2, types_1.Variant.TYPED);
});
describe('null type', () => {
...
describe('pair', () => {
    ...
    `;
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 3: Type 'null' is not assignable to type 'Pair<number, null>'."`);
    });
    it('type alias with the same name cannot be declared', () => {
        const code = 'type Pair = string;';
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 1: Type alias 'Pair' has already been declared."`);
    });
});
describe('list', () => {
    it('handles type mismatches correctly', () => {
    ...
    it('type alias with the same name cannot be declared', () => {
        ...
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 1: Type alias 'List' has already been declared."`);
    });
});
describe('head', () => {
    ...
    `;
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 4: Expected 1 arguments, but got 2."`);
    });
    ...
    `;
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 7: Type 'string | number' is not assignable to type 'boolean'."`);
    });
...
describe('tail', () => {
    ...
    `;
        parser_1.parse(code, context);
        expect(__1.parseError(context.errors)).toMatchInlineSnapshot(`"Line 7: Type 'List<string | number>' is not assignable to type 'List<boolean>'."`);
    });
});
