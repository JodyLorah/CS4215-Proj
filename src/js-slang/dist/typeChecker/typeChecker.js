...
function traverse(node, constraints) {
    ...
    if (constraints && node.typability !== 'Untypable') {
        ...
        node.inferredType = utils_2.tVar(`T${typeIdCounter}`);
        typeIdCounter++;
    }
    switch (node.type) {
        case 'Program': {
        ...
        case 'FunctionDeclaration': {
            ...
            if (constraints) {
                ...
                funcDeclNode.functionInferredType = utils_2.tVar(`T${typeIdCounter}`);
            }
            typeIdCounter++;
...
function typeCheck(program, context) {
    function typeCheck_(program) {
        ...
        if (context.chapter >= 3 && env.length === 3) {
            ...
            for (const [name, type] of utils_2.temporaryStreamFuncs) {
    ...
    for (const code of context.unTypecheckedCode) {
        typeCheck_(utils_1.typedParse(code, context));
    }
    ...
    switch (type.kind) {
        ...
        case 'pair': {
            ...
            if (pairTailType.kind === 'list' && pairHeadType === getListType(pairTailType)) {
                return utils_2.tList(pairHeadType);
            }
            else {
                return utils_2.tPair(pairHeadType, pairTailType);
            }
        }
        case 'list': {
...
function downgradePredicateToFunction(_type) {
    return {
        kind: 'function',
        parameterTypes: [freshTypeVar(utils_2.tVar('T'))],
        returnType: utils_2.tBool
    };
}
function genFreshPredicateType() {
    return utils_2.tFunc(freshTypeVar(utils_2.tVar('T')), utils_2.tBool);
}
function addToConstraintList(constraints, [LHS, RHS]) {
    ...
    if (LHS.kind === 'primitive' && RHS.kind === 'primitive' && LHS.name === RHS.name) {
        ...
        if ((RHS.kind === 'primitive' || RHS.kind === 'variable') && LHS.name === RHS.name) {
            ...
            addToConstraintList(constraints, [LHS, utils_2.tList(RHS.headType)]);
        }
            ...
            return addToConstraintList(newConstraints, [LHS, utils_2.tList(RHS.headType)]);
        }
        ...
        return addToConstraintList(constraints, [RHS, utils_2.tPair(LHS.elementType, LHS)]);
    }
    else if (RHS.kind === 'list' && LHS.kind === 'pair') {
        // if t = Pair<T1, T2> and t' = List<T3>, add t = Pair<T3, List<T3>>
        return addToConstraintList(constraints, [LHS, utils_2.tPair(RHS.elementType, RHS)]);
    }
...
function addPredicateTestToConstraintList(node, tests, consequent, isTopLevelAndLastValStmt, env, constraints) {
    ...
    for (const test of tests) {
        utils_2.setType(test.argVarName, freshTypeVar(utils_2.tVar('PredicatedT')), env);
    }
    // next apply the constraints from each type test
    for (const test of tests) {
        const argVarType = utils_2.lookupType(test.argVarName, env);
...
function extractPositiveTypeTests(node, env, result = []) {
    if (node.type === 'LogicalExpression' && node.operator === '&&') {
        ...
        const calleeType = utils_2.lookupType(node.callee.name, env);
...
function _infer(node, env, constraints, isTopLevelAndLastValStmt = false) {
    ...
    switch (node.type) {
        case 'UnaryExpression': {
            const op = node.operator === '-' ? utils_2.NEGATIVE_OP : node.operator;
            const funcType = utils_2.lookupType(op, env); // in either case its a monomorphic type
            ...
            try {
                newConstraints = addToConstraintList(newConstraints, [utils_2.tFunc(argType, storedType), funcType]);
            }
            catch (e) {
                if (e instanceof internalTypeErrors_1.UnifyError) {
        ...
        case 'LogicalExpression': {
            ...
            let newConstraints = constraints;
            newConstraints = addToConstraintList(constraints, [storedType, utils_2.tBool]);
            ...
            try {
                newConstraints = addToConstraintList(constraints, [leftType, utils_2.tBool]);
                newConstraints = addToConstraintList(constraints, [rightType, utils_2.tBool]);
            }
            catch (e) {
                if (e instanceof internalTypeErrors_1.UnifyError) {
                    addTypeError(new typeErrors_1.InvalidArgumentTypesError(node, [leftNode, rightNode], [utils_2.tBool, utils_2.tBool], [receivedLeftType, receivedRightType]));
                }
            }
            return newConstraints;
        }
        case 'BinaryExpression': {
            const envType = utils_2.lookupType(node.operator, env);
            ...
            try {
                newConstraints = addToConstraintList(constraints, [
                    utils_2.tFunc(leftType, rightType, storedType),
                    opType
                ]);
            }
            catch (e) {
        ...
        case 'ExpressionStatement': {
            return infer(node.expression, env, addToConstraintList(constraints, [storedType, utils_2.tUndef]));
        }
        case 'ReturnStatement': {
            const argNode = node.argument;
        ...
        case 'WhileStatement': {
            ...
            try {
                ...
                newConstraints = addToConstraintList(newConstraints, [testType, utils_2.tBool]);
            }
            catch (e) {
                if (e instanceof internalTypeErrors_1.UnifyError) {
        ...
        case 'ForStatement': {
            ...
            if (initNode.type === 'VariableDeclaration' &&
                ...
                utils_2.setType(initName, initNode.declarations[0].init.inferredType, env);
                utils_2.setDeclKind(initName, initNode.kind, env);
                newConstraints = infer(initNode, env, newConstraints);
                utils_2.setType(initName, utils_2.tForAll(applyConstraints(initNode.declarations[0].init
        ...
        case 'BlockStatement': {
                ...
                if (statement.type === 'ImportDeclaration') {
                    for (const specifier of statement.specifiers) {
                        if (specifier.type === 'ImportSpecifier' && specifier.local.type === 'Identifier') {
                            utils_2.setType(specifier.local.name, utils_2.tForAll(utils_2.tVar('T1')), env);
                            utils_2.setDeclKind(specifier.local.name, 'const', env);
                        }
                    }
                }
            }
            ...
            declNodes.forEach(declNode => {
                if (declNode.type === 'FunctionDeclaration' && declNode.id !== null) {
                    ...
                    utils_2.setType(declName, declNode.functionInferredType, env);
                    utils_2.setDeclKind(declName, 'const', env);
                }
                    ...
                    utils_2.setType(declName, declNode.declarations[0].init
                        .inferredType, env);
                    utils_2.setDeclKind(declName, declNode.kind, env);
                }
            });
            ...
            declNodes.forEach(declNode => {
                if (declNode.type === 'FunctionDeclaration' && declNode.id !== null) {
                    utils_2.setType(declNode.id.name, utils_2.tForAll(applyConstraints(declNode.functionInferredType, newConstraints)), env);
                }
                else if (declNode.type === 'VariableDeclaration' &&
                    declNode.declarations[0].id.type === 'Identifier') {
                    utils_2.setType(declNode.declarations[0].id.name, utils_2.tForAll(applyConstraints(declNode.declarations[0].init
                        .inferredType, newConstraints)), env);
                }
            });
        ...
        case 'Literal': {
            ...
            if (literalVal === null) {
                return addToConstraintList(constraints, [storedType, utils_2.tList(utils_2.tVar(`T${typeIdCounter++}`))]);
            }
            else if (typeOfLiteral === 'number') {
                return addToConstraintList(constraints, [storedType, utils_2.tNumber]);
            }
            else if (typeOfLiteral === 'boolean') {
                return addToConstraintList(constraints, [storedType, utils_2.tBool]);
            }
            else if (typeOfLiteral === 'string') {
                return addToConstraintList(constraints, [storedType, utils_2.tString]);
            }
            throw Error('Unexpected literal type');
        }
        case 'Identifier': {
            const identifierName = node.name;
            const envType = utils_2.lookupType(identifierName, env);
        ...
        case 'ArrowFunctionExpression': {
            ...
            const newConstraints = addToConstraintList(constraints, [storedType, utils_2.tFunc(...paramTypes)]);
            paramNodes.forEach((paramNode) => {
                utils_2.setType(paramNode.name, paramNode.inferredType, env);
            });
        ...
        case 'VariableDeclaration': {
            ...
            return infer(initNode, env, addToConstraintList(constraints, [storedType, utils_2.tUndef]));
        }
        case 'FunctionDeclaration': {
            const funcDeclNode = node;
            let newConstraints = addToConstraintList(constraints, [storedType, utils_2.tUndef]);
            utils_2.pushEnv(env);
        ...
        case 'CallExpression': {
            ...
            try {
                newConstraints = addToConstraintList(constraints, [utils_2.tFunc(...argTypes), calleeType]);
            }
            catch (e) {
                if (e instanceof internalTypeErrors_1.UnifyError) {
        ...
        case 'AssignmentExpression': {
            ...
            newConstraints = infer(rightNode, env, newConstraints);
            if (leftNode.type === 'Identifier' && utils_2.lookupDeclKind(leftNode.name, env) === 'const') {
            ...
            try {
                ...
                if (e instanceof internalTypeErrors_1.UnifyError) {
                    if (leftNode.type === 'Identifier') {
                        ...
                        addTypeError(new typeErrors_1.ArrayAssignmentError(node, utils_2.tArray(applyConstraints(leftType, newConstraints)), utils_2.tArray(applyConstraints(rightType, newConstraints))));
                    }
                }
            }
            return newConstraints;
        ...
        case 'ArrayExpression': {
            ...
            const arrayElementType = utils_2.tVar(`T${typeIdCounter++}`);
            newConstraints = addToConstraintList(newConstraints, [storedType, utils_2.tArray(arrayElementType)]);
            elements.forEach(element => {
                try {
                    newConstraints = addToConstraintList(newConstraints, [
                        arrayElementType,
                    ...
                    if (e instanceof internalTypeErrors_1.UnifyError) {
                        addTypeError(new typeErrors_1.ArrayAssignmentError(node, applyConstraints(node.inferredType, newConstraints), utils_2.tArray(applyConstraints(element.inferredType, newConstraints))));
                    }
                }
            });
            return newConstraints;
        ...
        case 'MemberExpression': {
            ...
            const envType = utils_2.lookupType(objName, env);
            ...
            try {
                newConstraints = addToConstraintList(constraints, [propertyType, utils_2.tNumber]);
            }
            catch (e) {
                if (e instanceof internalTypeErrors_1.UnifyError) {
        ...
        default:
            return addToConstraintList(constraints, [storedType, utils_2.tUndef]);
    }
}
