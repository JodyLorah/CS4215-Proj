...
function checkForTypeErrors(program, inputContext) {
    ...
    env = lodash_1.cloneDeep(context.typeEnvironment);
    // Override predeclared function types
    for (const [name, type] of utils_1.getTypeOverrides(context.chapter)) {
        utils_1.setType(name, type, env);
    }
    if (context.chapter >= 4) {
        // Add parse tree types to type environment
        const source4Types = parser_1.parse(parseTreeTypes_prelude_1.parseTreeTypesPrelude, {
            sourceType: 'module',
...
function typeCheckAndReturnType(node) {
    ...
    switch (node.type) {
        case 'Literal': {
            ...
            return utils_1.tPrimitive(typeof node.value, node.value);
        }
        case 'Identifier': {
        ...
        case 'FunctionDeclaration':
            ...
            if (hasVarArgs) {
                utils_1.setType(fnName, utils_1.tAny, env);
                return utils_1.tUndef;
            }
            const types = getParamTypes(params);
            // Return type will always be last item in types array
            types.push(expectedReturnType);
            const fnType = utils_1.tFunc(...types);
            // Typecheck function body, creating new environment to store arg types, return type and function type
            utils_1.pushEnv(env);
            params.forEach((param) => {
                utils_1.setType(param.name, getTypeAnnotationType(param.typeAnnotation), env);
            });
            // Set unique identifier so that typechecking can be carried out for return statements
            utils_1.setType(utils_1.RETURN_TYPE_IDENTIFIER, expectedReturnType, env);
            utils_1.setType(fnName, fnType, env);
            const actualReturnType = typeCheckAndReturnType(node.body);
            env.pop();
            if (lodash_1.isEqual(actualReturnType, utils_1.tVoid) &&
                !lodash_1.isEqual(expectedReturnType, utils_1.tAny) &&
                !lodash_1.isEqual(expectedReturnType, utils_1.tVoid)) {
        ...
        case 'VariableDeclaration': {
            ...
            const expectedType = env[env.length - 1].typeMap.has(id.name)
                ? (_a = lookupTypeAndRemoveForAllAndPredicateTypes(id.name)) !== null && _a !== void 0 ? _a : getTypeAnnotationType(id.typeAnnotation) : getTypeAnnotationType(id.typeAnnotation);
            ...
            utils_1.setType(id.name, expectedType, env);
            utils_1.setDeclKind(id.name, node.kind, env);
            return utils_1.tUndef;
        }
        case 'CallExpression': {
            ...
            if (context.chapter >= 2 && callee.type === 'Identifier') {
                ...
                if (fnName === 'list') {
                    ...
                    let pairType = utils_1.tPair(typeCheckAndReturnType(args[args.length - 1]), utils_1.tNull);
                    for (let i = args.length - 2; i >= 0; i--) {
                        pairType = utils_1.tPair(typeCheckAndReturnType(args[i]), pairType);
                    }
                    return utils_1.tList(elementType, pairType);
                }
                if (fnName === 'head' || fnName === 'tail') {
                    ...
                    const expectedType = utils_1.tUnion(utils_1.tPair(utils_1.tAny, utils_1.tAny), utils_1.tList(utils_1.tAny));
                ...
                if (fnName === 'stream' && context.chapter >= 3) {
                    ...
                    return utils_1.tStream(elementType);
                }
            }
            const calleeType = typeCheckAndReturnType(callee);
            if (calleeType.kind !== 'function') {
                if (calleeType.kind !== 'primitive' || calleeType.name !== 'any') {
                    context.errors.push(new typeErrors_1.TypeNotCallableError(node, utils_1.formatTypeString(calleeType)));
                }
                return utils_1.tAny;
            }
            ...
            for (let i = 0; i < expectedTypes.length; i++) {
                ...
                for (const mapping of mappings) {
                    const typeVar = utils_1.tVar(mapping[0]);
        ...
        case 'AssignmentExpression':
            ...
            if (node.left.type === 'Identifier' && utils_1.lookupDeclKind(node.left.name, env) === 'const') {
        ...
        case 'ArrayExpression':
            ...
            if (elements.length === 0) {
                return utils_1.tArray(utils_1.tAny);
            }
            const elementTypes = elements.map(elem => typeCheckAndReturnType(elem));
            return utils_1.tArray(mergeTypes(node, ...elementTypes));
        case 'MemberExpression':
            ...
            if (hasTypeMismatchErrors(node, indexType, utils_1.tNumber)) {
                context.errors.push(new typeErrors_1.InvalidIndexTypeError(node, utils_1.formatTypeString(indexType, true)));
            }
            // Expression being accessed must be array
            if (objectType.kind !== 'array') {
                context.errors.push(new typeErrors_1.InvalidArrayAccessTypeError(node, utils_1.formatTypeString(objectType)));
                return utils_1.tAny;
            }
        ...
        case 'TSAsExpression':
            ...
            if (hasTypeMismatchErrors(node, typeToCastTo, originalType)) {
                context.errors.push(new typeErrors_1.TypecastError(node, utils_1.formatTypeString(originalType, formatAsLiteral), utils_1.formatTypeString(typeToCastTo, formatAsLiteral)));
            }
            return typeToCastTo;
...
function handleImportDeclarations(node) {
    ...
    const modules = moduleLoader_1.memoizedGetModuleManifest();
    ...
    importStmts.forEach(stmt => {
        ...
        stmt.specifiers.map(spec => {
            ...
            utils_1.setType(spec.local.name, utils_1.tAny, env);
        });
    });
}
/**
    ...
    node.body.forEach(bodyNode => {
        switch (bodyNode.type) {
            ...
            case 'VariableDeclaration':
                ...
                // Save variable type and decl kind in type env
                utils_1.setType(id.name, expectedType, env);
                utils_1.setDeclKind(id.name, bodyNode.kind, env);
                break;
            case 'TSTypeAliasDeclaration':
                if (node.type === 'BlockStatement') {
                ...
                if (utils_1.lookupTypeAlias(alias, env) !== undefined) {
                ...
                if (bodyNode.typeParameters && bodyNode.typeParameters.params.length > 0) {
                    ...
                    bodyNode.typeParameters.params.forEach(param => {
                        ...
                        typeParams.push(utils_1.tVar(name));
                    });
                    type = utils_1.tForAll(getTypeAnnotationType(bodyNode), typeParams);
                    env.pop();
                }
                else {
                    type = getTypeAnnotationType(bodyNode);
                }
                utils_1.setTypeAlias(alias, type, env);
                break;
            default:
                break;
        }
...
function typeCheckAndReturnBinaryExpressionType(node) {
    const leftType = typeCheckAndReturnType(node.left);
    const rightType = typeCheckAndReturnType(node.right);
    const leftTypeString = utils_1.formatTypeString(leftType);
    const rightTypeString = utils_1.formatTypeString(rightType);
    const operator = node.operator;
    switch (operator) {
        case '-':
        case '*':
        ...
        case '+':
            ...
            checkForTypeMismatch(node, leftType, utils_1.tUnion(utils_1.tNumber, utils_1.tString));
            checkForTypeMismatch(node, rightType, utils_1.tUnion(utils_1.tNumber, utils_1.tString));
            // Return type is number | string if both left and right are neither number nor string
            return utils_1.tUnion(utils_1.tNumber, utils_1.tString);
        case '<':
        case '<=':
        case '>':
        case '>=':
...
function checkForTypeMismatch(node, actualType, expectedType) {
    ...
    if (hasTypeMismatchErrors(node, actualType, expectedType)) {
        context.errors.push(new typeErrors_1.TypeMismatchError(node, utils_1.formatTypeString(actualType, formatAsLiteral), utils_1.formatTypeString(expectedType, formatAsLiteral)));
    }
}
/**
...
function hasTypeMismatchErrors(node, actualType, expectedType, visitedTypeAliasesForActualType = [], visitedTypeAliasesForExpectedType = [], skipTypeAliasExpansion = false) {
    var _a;
    if (lodash_1.isEqual(actualType, utils_1.tAny) || lodash_1.isEqual(expectedType, utils_1.tAny)) {
    ...
    switch (expectedType.kind) {
        case 'variable':
            ...
            for (const visitedType of visitedTypeAliasesForExpectedType) {
                if (lodash_1.isEqual(visitedType, expectedType)) {
        ...
        case 'list':
            if (lodash_1.isEqual(actualType, utils_1.tNull)) {
        ...
        case 'array':
            if (actualType.kind === 'union') {
                ...
                return hasTypeMismatchErrors(node, utils_1.tUnion(...combinedType), expectedType.elementType, visitedTypeAliasesForActualType, visitedTypeAliasesForExpectedType, skipTypeAliasExpansion);
            }
...
function getAnnotatedType(typeNode) {
    switch (typeNode.type) {
        ...
        case 'TSLiteralType':
            ...
            return utils_1.tLiteral(value);
        case 'TSArrayType':
            return utils_1.tArray(getAnnotatedType(typeNode.elementType));
        case 'TSUnionType':
        ...
        case 'TSTypeReference':
            ...
            if (typeNode.typeParameters) {
                ...
                return utils_1.tVar(name, typesToSub);
            }
            return utils_1.tVar(name);
...
function getHeadType(node, type) {
    switch (type.kind) {
        ...
        case 'union':
            return utils_1.tUnion(...type.types.map(type => getHeadType(node, type)));
        case 'variable':
...
function getTailType(node, type) {
    switch (type.kind) {
        ...
        case 'list':
            return utils_1.tList(type.elementType, type.typeAsPair && type.typeAsPair.tailType.kind === 'pair'
                ? type.typeAsPair.tailType
                : undefined);
        case 'union':
            return utils_1.tUnion(...type.types.map(type => getTailType(node, type)));
        case 'variable':
...
function getBasicType(node) {
    ...
    return utils_1.tPrimitive(basicType);
}
/**
...
function lookupTypeAndRemoveForAllAndPredicateTypes(name) {
    const type = utils_1.lookupType(name, env);
    if (!type) {
        return undefined;
    }
    if (type.kind === 'forall') {
...
function lookupTypeAliasAndRemoveForAllTypes(node, varType) {
    ...
    const aliasType = utils_1.lookupTypeAlias(varType.name, env);
    if (!aliasType) {
    ...
    let polyType = lodash_1.cloneDeep(aliasType.polyType);
...
function substituteVariableTypes(type, typeVar, typeToSub) {
    switch (type.kind) {
        ...
        case 'literal':
            return type;
        case 'variable':
            if (lodash_1.isEqual(type, typeVar)) {
                return typeToSub;
            }
            if (type.typeArgs !== undefined) {
                for (let i = 0; i < type.typeArgs.length; i++) {
                    if (lodash_1.isEqual(type.typeArgs[i], typeVar)) {
        ...
        case 'function':
            ...
            return utils_1.tFunc(...types);
        case 'union':
            return utils_1.tUnion(...type.types.map(type => substituteVariableTypes(type, typeVar, typeToSub)));
        case 'pair':
            return utils_1.tPair(substituteVariableTypes(type.headType, typeVar, typeToSub), substituteVariableTypes(type.tailType, typeVar, typeToSub));
        case 'list':
            return utils_1.tList(substituteVariableTypes(type.elementType, typeVar, typeToSub), type.typeAsPair && substituteVariableTypes(type.typeAsPair, typeVar, typeToSub));
        case 'array':
            return utils_1.tArray(substituteVariableTypes(type.elementType, typeVar, typeToSub));
        default:
            return type;
    }
}
 ...
 */
function mergeTypes(node, ...types) {
    const mergedTypes = [];
    for (const currType of types) {
        if (lodash_1.isEqual(currType, utils_1.tAny)) {
            return utils_1.tAny;
        }
    ...
    return utils_1.tUnion(...mergedTypes);
}
/**
 * Checks if a type exists in an array of types.
 */
function containsType(node, arr, typeToCheck, visitedTypeAliasesForTypes = [], visitedTypeAliasesForTypeToCheck = [], skipTypeAliasExpansion = false) {
