...
exports.callIteratively = (f, nativeStorage, ...args) => {
    ...
    const pastCalls = [];
    while (true) {
        const dummy = astCreator_1.locationDummyNode(line, column, source);
        f = forceIt(f);
...
exports.setProp = (obj, prop, value, line, column, source) => {
    const dummy = astCreator_1.locationDummyNode(line, column, source);
...
exports.getProp = (obj, prop, line, column, source) => {
    const dummy = astCreator_1.locationDummyNode(line, column, source);
    const error = rttc.checkMemberAccess(dummy, obj, prop);
    if (error === undefined) {
