...
test('Transform import declarations into variable declarations', () => {
    ...
    foo(bar);
  `;
    const context = context_1.mockContext(types_1.Chapter.SOURCE_4);
    const program = parser_1.parse(code, context);
    const [, importNodes] = transpiler_1.transformImportDeclarations(program, new Set());
...
test('Transpiler accounts for user variable names when transforming import statements', () => {
  ...
  `;
    ...
    const program = parser_1.parse(code, context);
    const [, importNodes, [varDecl0, varDecl1]] = transpiler_1.transformImportDeclarations(program, new Set(['__MODULE_0__', '__MODULE_2__']));
    expect(importNodes[0].type).toBe('VariableDeclaration');
