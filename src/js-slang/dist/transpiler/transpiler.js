...
function transformImportDeclarations(program, usedIdentifiers, useThis = false) {
    const prefix = [];
    const [importNodes, otherNodes] = lodash_1.partition(program.body, node => node.type === 'ImportDeclaration');
    ...
    const declNodes = importNodes.flatMap(node => {
        ...
        if (!moduleNames.has(moduleName)) {
            ...
            moduleCount++;
            const moduleText = moduleLoader_1.memoizedGetModuleFile(moduleName, 'bundle').trim();
...
function transpileToFullJS(program, context, skipUndefined) {
    ...
    const transpiled = modulePrefix + astring_1.generate(transpiledProgram, { sourceMap });
