...
class SourceParser {
    ...
    static tokenize(programStr, context) {
        return [
            ...acorn_1.tokenizer(programStr, utils_1.createAcornParserOptions(constants_1.DEFAULT_ECMA_VERSION, context.errors))
        ];
    }
    parse(programStr, context, options, throwOnError) {
        try {
            return acorn_1.parse(programStr, utils_1.createAcornParserOptions(constants_1.DEFAULT_ECMA_VERSION, context.errors, options));
        }
        catch (error) {
    ...
    validate(ast, context, throwOnError) {
        ...
        walkers_1.ancestor(ast, mapToObj(validationWalkers), undefined, undefined);
