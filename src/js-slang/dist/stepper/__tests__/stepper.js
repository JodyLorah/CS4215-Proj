...
describe('Test codify works on non-circular abstract syntax graphs', () => {
  ...
  `; // not valid stepper input, but can be generated by stepper
        const program = parser_1.parse(code, context_1.mockContext(types_1.Chapter.SOURCE_4));
        expect(stepper_1.codify(program)).toMatchInlineSnapshot(`
      "[1, 2];
      "
    `);
...
test('Test two statement substitution', () => {
  ...
  `;
    const program = parser_1.parse(code, context_1.mockContext());
    const steps = stepper_1.getEvaluationSteps(program, context_1.mockContext(types_1.Chapter.SOURCE_4), 1000);
    expect(steps.map(x => stepper_1.codify(x[0])).join('\n')).toMatchInlineSnapshot(`
    "(1 + 2) * (3 + 4);
    3 * 5;
...
test('subsets', () => {
 ...
 subsets(list(1, 2, 3));
  `;
    const program = parser_1.parse(code, context_1.mockContext(types_1.Chapter.SOURCE_2));
    const steps = stepper_1.getEvaluationSteps(program, context_1.mockContext(types_1.Chapter.SOURCE_2), 1000);
    expect(steps.map(x => stepper_1.codify(x[0])).join('\n')).toMatchSnapshot();
});
// source 0
