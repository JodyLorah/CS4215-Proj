...
function findDeclaration(code, context, loc) {
    const program = utils_1.looseParse(code, context);
    if (!program) {
        return null;
    }
    const identifierNode = finder_1.findIdentifierNode(program, context, loc);
    if (!identifierNode) {
        return null;
    }
    const declarationNode = finder_1.findDeclarationNode(program, identifierNode);
...
function getScope(code, context, loc) {
    ...
    return scope_refactoring_1.getScopeHelper(declarationNode.loc, program, identifierNode.name);
}
...
function getAllOccurrencesInScope(code, context, loc) {
    ...
    return scope_refactoring_1.getAllOccurrencesInScopeHelper(declarationNode.loc, program, identifierNode.name);
}
...
function getNames(code, line, col, context) {
    return __awaiter(this, void 0, void 0, function* () {
        const [program, comments] = utils_2.parseWithComments(code);
        if (!program) {
            return [[], false];
        }
        const cursorLoc = { line, column: col };
        const [progNames, displaySuggestions] = name_extractor_1.getProgramNames(program, comments, cursorLoc);
        const keywords = name_extractor_1.getKeywords(program, cursorLoc, context);
...
function getTypeInformation(code, context, loc, name) {
    try {
        ...
        const program = utils_1.typedParse(code, context);
        if (program === null) {
            return '';
        }
        if (context.prelude !== null) {
            typeChecker_1.typeCheck(utils_1.typedParse(context.prelude, context), context);
        }
        const [typedProgram, error] = typeChecker_1.typeCheck(program, context);
        ...
        const res = walkers_1.findNodeAt(typedProgram, undefined, undefined, findByLocationPredicate);
        if (res === undefined) {
            return ans;
        }
        const node = res.node;
        ...
        const actualNode = node.type === 'VariableDeclaration'
            ...
            : node;
        const type = stringify_1.typeToString(actualNode.type === 'FunctionDeclaration'
...
function runFilesInContext(files, entrypointFilePath, context, options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        for (const filePath in files) {
            const filePathError = filePaths_1.validateFilePath(filePath);
        ...
        if (context.chapter === types_1.Chapter.FULL_JS || context.chapter === types_1.Chapter.FULL_TS) {
            const program = parser_1.parse(code, context);
            if (program === null) {
                return runner_1.resolvedErrorPromise;
            }
            return runner_1.fullJSRunner(program, context, options);
        }
        if (context.chapter === types_1.Chapter.HTML) {
            return runner_1.htmlRunner(code, context, options);
        }
        ...
        //        global state.
        verboseErrors = runner_1.hasVerboseErrors(code);
        return runner_1.sourceFilesRunner(files, entrypointFilePath, context, options);
    });
}
...
function resume(result) {
    if (result.status === 'finished' || result.status === 'error') {
        ...
        const value = interpreter_1.resumeEvaluate(result.context);
        return interpreter_1.ECEResultPromise(result.context, value);
    }
    else {
...
function compileFiles(files, entrypointFilePath, context, vmInternalFunctions) {
    ...
    try {
        return svml_compiler_1.compileToIns(preprocessedProgram, undefined, vmInternalFunctions);
    }
    catch (error) {
